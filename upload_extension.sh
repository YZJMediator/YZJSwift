...
Repo="Specs"
RepoList=("Specs" "Specs_test" "Specs_gray")
getRepo() {
    echo -e "\n"
    length=${#RepoList[@]}
...
DevelopVersionString=`grep -E 'version.develop.*=' YZJSwift_Extension.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' YZJSwift_Extension.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' YZJSwift_Extension.podspec`
...
if [ "$Repo" == "Specs_test" ]; then
  NewTestVersionNumber=$(($TestVersionNumber + 1))
fi

if [ "$Repo" == "Specs_gray" ]; then
  NewGrayVersionNumber=$(($GrayVersionNumber + 1))
fi

DevelopVersionLineNumber=`grep -nE 'version.develop.*=' YZJSwift_Extension.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" YZJSwift_Extension.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' YZJSwift_Extension.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" YZJSwift_Extension.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' YZJSwift_Extension.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" YZJSwift_Extension.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' YZJSwift_Extension.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' YZJSwift_Extension.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" YZJSwift_Extension.podspec
...
pod repo push ${Repo} YZJSwift_Extension.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
