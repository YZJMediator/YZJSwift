...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' YZJSwift.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" YZJSwift.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' YZJSwift.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" YZJSwift.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' YZJSwift.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" YZJSwift.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' YZJSwift.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' YZJSwift.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" YZJSwift.podspec
...
pod repo push ${Repo} YZJSwift.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
